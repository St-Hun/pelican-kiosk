...
// Rasa API 설정
const rasa = new Rasa('http://localhost:5005', 'default', 'http://localhost:5005');
...
app.post("/icemenu", (req, res) => {
  ...
  if (newItem.menuName && newItem.price) {
    data.iceMenu = [...data.iceMenu, newItem];
    res.send(data.iceMenu);
  } else {
...
app.post("/hotmenu", (req, res) => {
  const newItem = req.body;
  if (newItem.menuName && newItem.price) {
    data.hotMenu = [...data.hotMenu, newItem];
    res.send(data.hotMenu);
  } else {
    res.status(400).send({ error: "Invalid data" });
  }
...
app.post("/seasonMenu", (req, res) => {
  const newItem = req.body;
  if (newItem.menuName && newItem.price) {
    data.seasonMenu = [...data.seasonMenu, newItem];
    res.send(data.seasonMenu);
  } else {
    res.status(400).send({ error: "Invalid data" });
  }
...
app.post("/chat", async (req, res) => {
  ...
  try {
    // Rasa API를 사용하여 Rasa 서버와 상호작용
    const response = await rasa.parse(message);
    res.json(response);
  } catch (error) {
    console.error("Error connecting to Rasa:", error);
    res.status(500).send("Error connecting to Rasa server");
  }
});
